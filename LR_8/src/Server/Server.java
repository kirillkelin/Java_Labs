...
// Класс controller, в которым реализованы методы, которые осуществляют общение между клиентом и сервером
public class Server {
    ...
    protected void acceptServer() {
        while (true) {
            try {
                ...
                new ServerThread(socket).start(); // для каждого клиента запускается поток выполнения
            } catch (Exception e) {
